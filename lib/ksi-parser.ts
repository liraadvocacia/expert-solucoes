...
/**
 * Normaliza o texto bruto do pdfjs-dist:
 *  - colapsa múltiplos espaços/tabs em um único espaço (mantém \n)
 *  - reduz sequências de 3+ newlines para 2
 * Isso garante que regex com \s+ funcionem mesmo quando o pdfjs
 * produz espaços extras entre itens de texto.
 */
function normalizar(raw: string): string {
  return raw
    .replace(/[^\S\n]+/g, " ")   // espaços/tabs múltiplos → 1 espaço (mantém \n)
    .replace(/\n{3,}/g, "\n\n"); // 3+ newlines → 2
}
...
export function parseKsiText(rawText: string): Partial<DadosRating> {
  // Normaliza antes de qualquer regex para garantir espaçamento previsível
  ...
  const nomeM = text.match(
    /\bNome\s+([A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ\s]{4,70}?)\s{2,}N[uú]mero/i
  );
  if (nomeM) {
    nomeCliente = nomeM[1].trim();
  } else {
    const fbM = text.match(
      /Nome(?:\s+do\s+Analisado)?[:\s]+([A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][^\n\r]{4,70})/i
    );
  ...
  const descM = text.match(
    /Conclus[aã]o\s+de\s+An[aá]lise\s+Inteligente[:\s]+([A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][^\s\n\r][^\n\r]{0,40})/i
  );
  if (descM) {
  ...
  const comprM =
    ...
    text.match(/Comprometimento[:\s]+([\d]+(?:[.,]\d+)?)\s*%/i);
  ...
  const capM =
    ...
    text.match(/Capacidade\s+Mensal\s+de\s+Pagamento[:\s]*R?\$?[:\s]*([\d.,]+)/i) ??
    text.match(/Capacidade\s+(?:de\s+)?[Pp]agamento\s+[Mm]ensal[:\s]*R?\$?[:\s]*([\d.,]+)/i) ??
    text.match(/Capacidade\s+Mensal[:\s]+R?\$?[:\s]*([\d.,]+)/i);
  ...
  const rendaM =
    ...
    text.match(/Renda\s+Presumida[:\s]+([\d.,]+)/i);
  ...
  const pontM =
    text.match(/Pontualidade\s+de\s+Pagamento[\s:]+(\d+(?:[.,]\d+)?)/i) ??
    text.match(/Pontualidade\s+Pagamento[\s:]+(\d+(?:[.,]\d+)?)/i) ??
    text.match(/Pontualidade[\s:]+(\d+(?:[.,]\d+)?)/i);
  ...
  const rgiDetailRe =
    /(NAO\s+INFORMADO|\d{2}\/\d{2}\/\d{4})\s+([A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][\w\sÁÉÍÓÚÀÂÊÔÃÕÜÇáéíóúàâêôãõüç.'-]{2,60}?)\s+R\$[:\s]*([\d.,]+)/gi;
  let m: RegExpExecArray | null;
  while ((m = rgiDetailRe.exec(text)) !== null) {
    const dataRef = /NAO\s+INFORMADO/i.test(m[1]) ? "Não informado" : m[1];
  ...
  const protestoDetailRe =
    /\b(\d{2,3}-[A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][\w\sÁÉÍÓÚÀÂÊÔÃÕÜÇáéíóúàâêôãõüç.'-]{5,120}?)\s+R\$[:\s]*([\d.,]+)/gi;
